...
class phpFlickr {
  var $api_key;
  var $secret;
  var $REST = 'api.flickr.com/services/rest/';
  var $Upload = 'api.flickr.com/services/upload/';
  var $Replace = 'api.flickr.com/services/replace/';
  var $req;
  var $response;
  var $parsed_response;
  var $cache = false;
  ...
  function phpFlickr ($api_key, $secret = NULL, $die_on_error = false) {
    ...
    $this->req = new HTTPRequest('api.flickr.com');
  ...
  function request ($command, $args = array(), $nocache = false) {
    ...
    $this->req->setRequestURI('/services/rest/');
    ...
    ksort($args);
    $auth_sig = "";
    if ( !$this->cache || !($this->response = $this->getCached($args)) || $nocache) {
